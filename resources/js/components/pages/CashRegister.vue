    ...
    <main>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm mb-6">
            <p class="font-semibold text-default-900 mb-3 flex items-center gap-2">
                <i class="fa-solid fa-sliders"></i> Ajustement manuel (écart de comptage)
            </p>
            <form @submit.prevent="submitAdjustment" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                ...
                <button type="submit" :disabled="adjusting"
                    ...
                    <i class="fa-solid fa-circle-check me-1"></i> {{ adjusting ? 'Enregistrement...' : 'Enregistrer' }}
                </button>
            </form>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
            <div class="flex items-center justify-between flex-wrap gap-3 mb-4 px-2 pt-2">
                <p class="font-semibold text-default-900 flex items-center gap-2">
                    <i class="fa-solid fa-clock-rotate-left"></i> Historique de caisse
                </p>
                <button @click="exportHistoryToExcel"
                    class="px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-sm rounded-md shadow-sm">
                    <i class="fa-solid fa-file-arrow-down me-1"></i> Exporter en Excel
                </button>
            </div>

            <!-- Point 6 : filtre de dates + devise -->
            <div class="flex flex-col sm:flex-row gap-3 mb-4 px-2">
                <div class="flex-1">
                    <label class="block text-xs text-gray-500 mb-1">Devise</label>
                    <select v-model="filters.currency_id" @change="loadHistory" class="w-full border border-gray-300 rounded-md p-2 text-sm">
                ...
                <div class="flex-1">
                    ...
                    <input type="date" v-model="filters.from" @change="loadHistory" class="w-full border border-gray-300 rounded-md p-2 text-sm">
                </div>
                <div class="flex-1">
                    <label class="block text-xs text-gray-500 mb-1">Au</label>
                    <input type="date" v-model="filters.to" @change="loadHistory" class="w-full border border-gray-300 rounded-md p-2 text-sm">
                </div>
            </div>
