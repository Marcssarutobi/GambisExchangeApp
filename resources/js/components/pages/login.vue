                    ...
                    <form style="width: 100%;" @submit.prevent="LoginForm">
                        <div class="mb-3">
                            <label for="exampleInputEmail1"
                                class="text-default-800 text-sm font-medium inline-block mb-2">Email address</label>
                            <input type="email" :class="isEmpty.email ? 'is-invalid border border-danger' : ''"
                                v-model="dataLogin.email" class="form-input" id="exampleInputEmail1"
                                aria-describedby="emailHelp" placeholder="Enter email">
                            <div v-if="isEmpty.email" class="invalid-feedback"
                                style="color: red; font-size: 12px; margin-top: 5px;">{{ msgInput.email }}</div>
                        </div>
                        <div class="mb-4">
                            <label for="exampleInputPassword1"
                                class="text-default-800 text-sm font-medium inline-block mb-2">Password</label>
                            <input type="password" :class="isEmpty.password ? 'is-invalid border border-danger' : ''"
                                v-model="dataLogin.password" class="form-input" id="exampleInputPassword1"
                                placeholder="Password">
                            <div v-if="isEmpty.password" class="invalid-feedback"
                                style="color: red; font-size: 12px; margin-top: 5px;">{{ msgInput.password }}</div>
                        </div>
                        <div class="">
                            <div class="d-flex justify-content-between gap-2"
                                ...
                                <router-link to="/forgot-password" class="text-primary-600 fw-medium">Forgot
                                    Password?</router-link>
                            </div>
                        </div>

                        <button disabled v-if="isLoader"
